    ...
    override fun getAll(): List<Variation> {
        ...
        return variationQueries.getAll().executeAsList().map { variation ->
            Variation(
                ...
                oneRepMax =
                    sets.filter { it.variationId == variation.id && it.reps == 1L }
                        .maxByOrNull { it.weight },
                eMax =
                    sets.filter { it.variationId == variation.id && it.reps > 1 }
...
private fun comliftbrodb.Lift.toDomain() =
    Lift(
        id = this.id,
        name = this.name,
...
private fun comliftbrodb.GetAll.toDomain(): Variation =
    Variation(
        id = this.id,
        lift =
            Lift(
...
private fun comliftbrodb.GetAllByVariation.toDomain(): LBSet =
    LBSet(
        id = this.id,
        ...
        reps = this.reps ?: 1,
        tempo =
            com.lift.bro.domain.models.Tempo(
